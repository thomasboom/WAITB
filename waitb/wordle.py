...
def load_words():
    ...
    valid_path = os.path.join(words_dir, "valid.txt")
    target_path = os.path.join(words_dir, "target.txt")
    
    if os.path.exists(valid_path):
        with open(valid_path, "r") as f:
            VALID_GUESSES = set(line.strip().upper() for line in f if line.strip())
    else:
        VALID_GUESSES = None
